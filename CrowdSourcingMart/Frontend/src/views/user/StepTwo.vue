      ...
      <div class="header1">
        <div class="page-header1">
          <div class="header1-container">
            <div class="avatar">
              <img
                src="https://timgsa.baidu.com/timg?image&quality=80&size=b9999_10000&sec=1509041832135&di=abc5c99b8048b0c600049e68cdb64602&imgtype=0&src=http%3A%2F%2Fimglf1.ph.126.net%2FnMbQAfw2LAJG4ahCYE_XJA%3D%3D%2F3886606478520925399.jpg">
            </div>
        ...
        <div id="rightformstyle" class="mycontent" :model="dynamicForm" :ref="dynamicForm" v-if="rightcontent">
            ...
            <div class="step-set-list">
              <el-collapse class="step-set" v-for="(step, indexout) in dynamicForm.steps" :key="step.key"
                ...
                <el-collapse-item>
                  <template slot="title">
                    <div class="input-box">
                      <el-input v-on:keyup.space.native="lockStep" v-on:keydown.space.native="lockStep" size="small"
                                placeholder="请输入内容" v-model="step.name" class="step-input">
                      ...
                      <el-form class="step-set-list-1" style="order: 1" :inline="true"
                        ...
                        <el-form-item class="form-item-style" label="数据字段">
                          ...
                          <div class="form-content-style">
                            ...
                            <set-det type="ilf" :dataFields="ilf.dataFields" :ilf="ilf.name" :tableData1="tableData1"
                                     :indexout="indexout" :indexmid="indexmid" :dynamicForm="dynamicForm"></set-det>
                            <!--</div>-->
                          </div>
                        </el-form-item>
                      ...
                      <el-form class="step-set-list-1" style="order: 1" :inline="true"
                        ...
                        <el-form-item class="form-item-style" label="数据字段">
                          <el-button size="mini" type="success"
                            ...
                            class="el-icon-plus"></i></el-button>
                          <set-det type="eif" :dataFields="eif.dataFields" :dynamicForm="dynamicForm" :ilf="eif.name"
                                   :tableData1="tableData2" :indexout="indexout" :indexmid="indexmid"></set-det>
          ...
          <div style="margin-top: 50px ;font-size: 13px;width: 100%;min-width:500px">
            ...
            <div class="style-bottom-btn">
              ...
              <el-button class="bottom-btn1" v-on:click="toStepOne" id="backbtn">上一步</el-button>
              <el-button id='saveTablesBtn' class="bottom-btn2" type="primary" v-on:click="saveTable">保存</el-button>
              <el-button id='NextBtn' class="bottom-btn3" type="primary" v-on:click="toStepThree">下一步</el-button>
        ...
        <div id="default-right" style="margin-left: 20px" class="mycontent" v-else>
          ...
          <div class='typora-export'>
            <div id='write' class='is-mac show-fences-line-number'>
              ...
              <br>
              <h3>3.划分功能</h3>
              <p>像把文件划分进文件夹中一样，将软件的所有 <a style="color: #4183C4;"
                                         href='http://10.60.38.173:8989/shikun/mart/wikis/function-decomposition#功能点'>功能点</a>
                划分进上一步建立的构架中。在右键菜单中选择
                <strong>新建子功能</strong> 或者 <strong>新建同级功能</strong></p>
              <img style="width: 35%" src="../../assets/functiontree.gif"/>

              <br>
              <h3>4.添加内部逻辑文件和外部接口文件</h3>
              <p>双击左侧的任一功能点，在右边第一行填写本功能点所用到的（也可以在下面步骤分解时填写）所有 <a style="color: #4183C4;"
                                                                 href='http://10.60.38.173:8989/shikun/mart/wikis/function-decomposition#内部逻辑文件ilf'>内部逻辑文件</a>
                和 <a style="color: #4183C4;"
                     href='http://10.60.38.173:8989/shikun/mart/wikis/function-decomposition#外部接口文件eif'>外部接口文件</a>
                ，文件名之间用空格隔开</p>
              ...
              <br>
              <h3>6.填写步骤所用文件</h3>
              <p>点击步骤框任意位置，在下拉框中填写该步骤用到的 <a style="color: #4183C4;"
                                            href='http://10.60.38.173:8989/shikun/mart/wikis/function-decomposition#内部逻辑文件ilf'>内部逻辑文件</a>
                和 <a style="color: #4183C4;"
                     href='http://10.60.38.173:8989/shikun/mart/wikis/function-decomposition#外部接口文件eif'>外部接口文件</a> ，点击 +
                增加内部逻辑文件 或者外部接口文件 。
              ...
              <br>
              <h3>7.填写文件所用数据字段</h3>
              <p>对上面选择的每一个<a style="color: #4183C4;"
                             href='http://10.60.38.173:8989/shikun/mart/wikis/function-decomposition#内部逻辑文件ilf'>内部逻辑文件</a>或
                <a style="color: #4183C4;"
                   href='http://10.60.38.173:8989/shikun/mart/wikis/function-decomposition#外部接口文件eif'>外部接口文件</a>
                填写此文件所用到的 <a style="color: #4183C4;"
                             href='http://10.60.38.173:8989/shikun/mart/wikis/function-decomposition#数据字段'>数据字段</a></p>
          ...
          <div class="style-bottom-btn">
            <el-button id="onebackbtn" class="bottom-btn1" v-on:click="toStepOne" >上一步</el-button>
            <el-button id="onenextbtn" class="bottom-btn3" type="primary" v-on:click="defaultToStepThree">下一步</el-button>
          </div>
          <div style="height: 50px;"></div>
        </div>
      </div>
...
//      },
    ...
    computed: {
      ...
      //     let k = 0;
      //     for(let i=0;i<this.tableData1.length;i++) {
      //         let len = this.tableData1[i].DET.trim().split(' ').length;
      //         for(let j = 0; j < len; j++) {
      ...
      eifDets: function () {
        ...
        let k = 0;
        for (let i = 0; i < this.tableData2.length; i++) {
          let len = this.tableData2[i].DET.trim().split(' ').length;
          for (let j = 0; j < len; j++) {
    ...
    methods: {
      ...
      saveTable() {
        ...
        this.$http.post(this.url + '/updateTransaction/' + this.$route.params.rId, msg).then(res => {
          console.log(msg);
          console.log(this.dynamicForm.steps);
          console.log(this.tableData1);
...
//                 })
      ...
      removeStep(item) {
        let index = this.dynamicForm.steps.indexOf(item)
        if (index !== -1) {
      ...
      //     let index = this.dynamicForm.steps[index_out].ilfs[index_mid].dataFields.indexOf(item)
      //     if (index !== -1) {
      ...
      //     let index = this.dynamicForm.steps[index_out].eifs[index_mid].dataFields.indexOf(item)
      //     if (index !== -1) {
      ...
      removeIlf(index_out, item) {
        let index = this.dynamicForm.steps[index_out].ilfs.indexOf(item)
        if (index !== -1) {
      ...
      removeEif(index_out, item) {
        let index = this.dynamicForm.steps[index_out].eifs.indexOf(item)
        if (index !== -1) {
...
// p_contextMenu: Object containing all the context menus. Set null for no context menu;
      createTre(vue, p_div, p_backColor, p_contextMenu, p_id = null) {
        ...
        let tempTreeId = (p_id === null) ? Date.now() : p_id;
        ...
        let tree = {
          ...
          createNode(vue, p_text, p_expanded, p_icon, p_parentNode, p_tag, p_contextmenu, p_id = null, p_type) {
            vue.v_tree = this;
            let tempId = (p_id === null) ? Date.now() : p_id;
              ...
              if (p_parentNode == undefined) {
                ...
                if (p_parentNode.childNodes.length == 0) {
                  ...
                  if (p_parentNode.expanded) {
                    ...
                    let v_img = p_parentNode.elementLi.getElementsByTagName("i")[0];
...
//                    //交换父子节点的ID
//                    let tempId=p_parentNode.id;
////                    let tempName=p_parentNode.text;
//
...
////                    vue.node.text=tempName;
          ...
          drawTree(vue) {
            ...
            let ulElement = this.createSimpleElement('ul', this.name, 'tree');
          ...
          drawNode(vue, p_ulElement, p_node) {
            ...
            let v_span = vue.createSimpleElement('span', vue.spanId + '_span_node', 'node');
            vue.spanId++;
            ...
            let v_a = vue.createSimpleElement('a', vue.testId + '_a', null);
            vue.testId++;
            ...
            let v_ul = vue.createSimpleElement('ul', 'ul_' + p_node.id, null);
          ...
          expandNode(p_node) {
            if (p_node.type === 'folder' && p_node.expanded == false) {
              ...
              let img = p_node.elementLi.getElementsByTagName("i")[0];
            ...
            if (p_node.type == 'folder') {
              ...
              if (vue.isModify === true) {
                vue.$confirm('是否保存当前内容?', '提示', {
                  ...
                  console.log(msg)
                  vue.$http.post(vue.url + '/updateTransaction/' + vue.$route.params.rId, msg,{headers: {Authorization: 'adminadmin'}}).then(res => {
                    //alert("success999")
                    ...
                    vue.$http.post(vue.url + '/getTransaction/' + vue.$route.params.rId, msg,{headers: {Authorization: 'adminadmin'}}).then(res => {
                      //alert("success")
...
//                    }
                      ...
                      for (let i = 0; i < res.body.steps.length; i++) {
                        ...
                        for (let j = 0; j < res.body.steps[i].ilfDataSets.length; j++) {
                          vue.dynamicForm.steps[i].ilfs.push({
                            "name": res.body.steps[i].ilfDataSets[j].innerlogicalFileName,
                            "dataFields": []
                          });
                          for (let k = 0; k < res.body.steps[i].ilfDataSets[j].DET.length; k++) {
                        ...
                        for (let j = 0; j < res.body.steps[i].eifDataSets.length; j++) {
                          vue.dynamicForm.steps[i].eifs.push({
                            "name": res.body.steps[i].eifDataSets[j].externalInterfaceFileName,
                            "dataFields": []
                          });
                          for (let k = 0; k < res.body.steps[i].eifDataSets[j].DET.length; k++) {
...
//                console.log(vue.dynamicForm);
          ...
          removeChildNodes(p_node) {
            ...
            if (p_node.type === 'folder') {
              ...
              let v_img = p_node.elementLi.getElementsByTagName("i")[0];
          ...
          nodeContextMenu(vue, p_event, p_node) {
            if (p_event.button === 2) {
              ...
              if (p_node.contextMenu !== undefined) {
                ...
                let v_menu = this.contextMenu[p_node.contextMenu];
                ...
                let v_left = p_event.pageX - 5;
                let v_right = p_event.pageY - 5;
                ...
                for (let i = 0; i < v_menu.elements.length; i++) (function (i) {

                  let v_li = vue.createSimpleElement('li', null, null);


                  let v_span = vue.createSimpleElement('span', i + '_spans', null);
                  v_span.onclick = function () {
                    v_menu.elements[i].action(vue, p_node)
                  };

                  let v_a = vue.createSimpleElement('a', null, null);
                  let v_ul = vue.createSimpleElement('ul', null, 'mymenu');
                  ...
                  if (v_menu.elements[i].icon != undefined) {
                    // let v_img = vue.createImgElement('null', v_menu.elements[i].icon.className + ' el-icon-menu');
                    let v_img = vue.createImgElement('null', "mymenu_img " + v_menu.elements[i].icon);

                    // let v_img = vue.createImgElement('null', v_menu.elements[i].icon.classNam);
...
//                                         let v_span_more = vue.createSimpleElement('div', null, null);
...
//                                     }
          ...
          contextMenuLi(vue, p_submenu, p_ul, p_node) {
            ...
            for (let i = 0; i < p_submenu.elements.length; i++) (function (i) {

              let v_li = vue.createSimpleElement('li', null, null);

              let v_span = vue.createSimpleElement('span', null, null);
              v_span.onclick = function () {
              ...
              if (p_submenu.elements[i].icon != undefined) {
                let v_img = vue.createImgElement('null', p_submenu.elements[i].icon);
                v_li.appendChild(v_img);
              }
            ...
            elements: [
              {
                ...
                action: function (vue, node) {
                  ...
                  vue.$prompt('输入名称', '提示', {
                    ...
                    let tempNode = node.createChildNode(vue, value, false, ' el-icon-plus', null, 'context1', null, 'file');
              ...
              },
              {
                ...
                action: function (vue, node) {
                  ...
                  vue.$prompt('输入名称', '提示', {
                    ...
                    let tempNode = node.createChildNode(vue, value, false, ' el-icon-plus', null, 'context1', null, 'folder');
...
//                        }
              ...
              },
              {
                ...
                action: function (vue, node) {
                  ...
                  let tempNode = node.createChildNode(vue, vue.cutNode.text, true, vue.cutNode.icon, node, 'context1', vue.cutNode.id, vue.cutNode.type);
                  ...
                  // let right=vue.returnNodeInSubtree(node);
                  // console.log(right)
                  ...
                  function pasteChildNode(vue, parentNode, cutNode) {
                    if (cutNode.childNodes.length != 0) {
                      for (let i = 0; i < cutNode.childNodes.length; i++) {
...
//                            console.log(Msg)
        ...
        // console.log(this.url + '/getTree/' + this.$route.params.rId)
        this.$http.get(this.url + '/getTree/' + this.$route.params.rId,{headers: {Authorization: 'adminadmin'}}).then(res => {
          ...
          //     {
          //       "fileName": "估算任务信息",
          //       "allDET": "任务ID 任务创建人 项目描述ID 项目模块ID 调整因子ID 任务状态 创建时间 备注信息 项目成本 时间成本 UFP AFP"
          //     },
          //     {
          //       "fileName": "项目描述信息",
          //       "allDET": "项目描述ID 项目名 项目负责人 联系方式 项目描述 估算方法"
          //     },
          //     {"fileName": "项目模块", "allDET": "项目模块ID 父模块ID 项目模块名"},
          //     {"fileName": "项目功能", "allDET": "项目功能ID 功能名称 父模块ID"},
          //     {"fileName": "功能步骤信息", "allDET": "项目步骤ID 项目功能ID 步骤名称"},
          //     {"fileName": "功能步骤内部逻辑文件信息", "allDET": "内部逻辑文件ID 项目步骤ID 内部逻辑文件名称 数据字段"},
          //     {"fileName": "功能步骤外部接口文件信息", "allDET": "外部接口文件ID 项目步骤ID 外部接口文件名称 数据字段"},
          //     {"fileName": "事务功能点", "allDET": "事务功能点ID 事务名称 事务类型 逻辑文件 数据字段 逻辑文件数量 数据字段数量 复杂度 UFP"},
          //     {"fileName": "数据功能点", "allDET": "数据功能点ID 估算任务ID 数据功能点名称 文件类型 记录文件 数据字段 记录文件数量 数据字段数量 复杂度 UFP"},
          //     {"fileName": "总内部逻辑文件信息", "allDET": "内部逻辑文件ID 文件名 数据字段 估算任务ID"},
          //     {"fileName": "总外部接口文件信息", "allDET": "外部接口文件ID 文件名 数据字段 估算任务ID"},
          //     {"fileName": "调整因子", "allDET": "调整因子ID 开发类型 开发平台 人均成本 人均效率 进度需求 延迟需求 复杂度 可靠性 是否使用数据库 语言类型"}
          //   ]
          // };
          ...
          for (let i = 0; i < treeMsg.ilfTable.length; i++) {
            this.tableData1.push({
              name: treeMsg.ilfTable[i].fileName,
              DET: treeMsg.ilfTable[i].allDET
            })
          }
          for (let i = 0; i < treeMsg.eifTable.length; i++) {
            this.tableData2.push({
              name: treeMsg.eifTable[i].fileName,
              DET: treeMsg.eifTable[i].allDET
            })
          }
          for (let i = 0; i < treeMsg.childFolders.length; i++) {
            let node1 = this.tree.createNode(this, treeMsg.childFolders[i].name, false, ' el-icon-close', null, null, 'context1', treeMsg.childFolders[i].id, 'folder');

            treeRecursion(this, node1, treeMsg.childFolders[i]);


          }
          for (let i = 0; i < treeMsg.childFiles.length; i++) {
            let node1 = this.tree.createNode(this, treeMsg.childFiles[i].name, false, ' el-icon-close', null, null, 'context1', treeMsg.childFiles[i].id, 'file');
...
//            }
        ...
        function treeRecursion(vue, fatherNode, curNode) {
          ...
          for (let i = 0; i < curNode.childFolders.length; i++) {
            let node2 = fatherNode.createChildNode(vue, curNode.childFolders[i].name, false, ' el-icon-close', null, 'context1', curNode.childFolders[i].id, 'folder');
            treeRecursion(vue, node2, curNode.childFolders[i]);
          }

          for (let i = 0; i < curNode.childFiles.length; i++) {
            let node2 = fatherNode.createChildNode(vue, curNode.childFiles[i].name, false, ' el-icon-close', null, 'context1', curNode.childFiles[i].id, 'file');
          }
