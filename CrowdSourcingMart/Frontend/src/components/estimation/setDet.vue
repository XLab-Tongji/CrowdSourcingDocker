    ...
    export default {
        ...
        methods:{
            removeDataField(index_out, index_mid, item) {
                if(this.type==="ilf") {
                  var index = this.dynamicForm.steps[index_out].ilfs[index_mid].dataFields.map(function(e) { return e.value; }).indexOf(item.value)
                  if (index !== -1) {
                  ...
                  var index = this.dynamicForm.steps[index_out].eifs[index_mid].dataFields.map(function(e) { return e.value; }).indexOf(item.value)
                        if (index !== -1) {
